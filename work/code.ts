...
figma.ui.onmessage = async (msg) => {
    ...
    if (msg.type === 'scan-page') {
        // Даем интерфейсу долю секунды, чтобы он успел показать текст "Preparing..." 
        // ДО того, как плагин заблокирует память поиском узлов
        await yieldToUI();
        ...
        for (let i = 0; i < total; i++) {
            ...
            // Отправляем сигнал интерфейсу каждые 2 картинки для плавности
            if (i % 2 === 0 || i === total - 1) {
